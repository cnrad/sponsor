...
export async function getServerSideProps() {
    const fetchedUserInfo: Record<string, string | number | boolean | null> = await fetch(
        `https://api.github.com/users/${information.username}`
    ).then(res => res.json());

    let fetchedFeaturedRepos: Array<Record<any, any>> = [];

    for await (const item of information.featuredRepos) {
        await fetch(`https://api.github.com/repos/${information.username}/${item}`)
            .then(res => res.json())
            .then(data => fetchedFeaturedRepos.push(data));
    }

    return {
        props: {
            fetchedUserInfo,
            fetchedFeaturedRepos,
        },
    };
}

const Home: NextPage = ({ fetchedUserInfo, fetchedFeaturedRepos }: any) => {
        ...
        <>
            ...
            <div className="w-[25rem] md:w-[36rem] h-auto flex flex-col items-center justify-center">
                ...
                <motion.div className="flex flex-row mt-4">
                    ...
                    <motion.button
                        className="text-[#c9d1d9] px-3 py-1 text-md rounded-md"
